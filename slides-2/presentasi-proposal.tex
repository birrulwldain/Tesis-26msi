...
\title[CNN--Transformer LIBS]{%
  PENGEMBANGAN MODEL CNN–TRANSFORMER \\
BERBASIS FISIKA UNTUK ANALISIS\\
KUANTITATIF LIBS PADA TANAH VULKANIK}
...
\begin{frame}{Previous Research}
  ...
  \begin{tikzpicture}[node distance=0.2cm and 0.4cm]
    ...
    \node[impbox, below=0.3cm of g1.south west, anchor=north west] (imp1) {Implikasi Tesis: Butuh \textit{Machine Learning} untuk iterasi \textit{inverse} mereduksi beban analitik.};
...
\begin{frame}{Proposed Framework Overview}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.56\textwidth}
      \centering
      \includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{fig-p/diagram-konseptual.png}
    \end{column}
    \begin{column}{0.42\textwidth}
      {\scriptsize
      \textbf{Why encoder--decoder?}
      \begin{itemize}\setlength{\itemsep}{1pt}
        \item Spektrum poliatom = \emph{komposisi} spektrum monoatom.
        \item \textit{Cross-attention} decoder mempelajari aturan komposisi.
        \item CNN: pola lokal; Transformer: dependensi global.
      \end{itemize}

      \vspace{0.3em}
      \textbf{Kerangka kerja (3 langkah):}
      \begin{enumerate}\setlength{\itemsep}{1pt}
        \item Bangkitkan spektrum sintetis mono \& poliatom (LTE).
        \item Latih CNN--Transformer Encoder--Decoder + \textit{cross-attention}.
        \item Validasi pada LIBS tanah vulkanik Aceh.
      \end{enumerate}
      }
    \end{column}
  \end{columns}
...
\begin{frame}{Sumber Data}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.30\textwidth}
      \centering
      \includegraphics[width=\textwidth,height=0.55\textheight,keepaspectratio]{fig-p/LIBS.png}\\[2pt]
      {\scriptsize\textit{Setup eksperimen LIBS,\\Lab Fisika USK}}
    \end{column}
    \begin{column}{0.67\textwidth}
      ...
      \begin{tikzpicture}[node distance=0.25cm and 0.45cm]
        ...
        \node[box, below=0.45cm of pre] (ft) {Fine-tuning};
        \node[box, left=of ft] (exp) {LIBS\\Eksperimental};
        \draw[arr] (sim) -- (syn);
        \draw[arr] (syn) -- (pre);
        \draw[arr] (pre) -- (ft);
        \draw[arr] (exp) -- (ft);
      ...
      \begin{columns}[T]
        \begin{column}{0.48\textwidth}
          ...
          \begin{itemize}\setlength{\itemsep}{1pt}
            \item Saha--Boltzmann (LTE).
            \item Transisi atomik NIST ASD.
            \item $T$: 5000--15\,000\,K.
            \item $n_e$: $10^{16}$--$10^{18}$\,cm$^{-3}$.
            \item \textit{Pre-training} model.
          \end{itemize}
          }
        \end{column}
        \begin{column}{0.48\textwidth}
          ...
          \begin{itemize}\setlength{\itemsep}{1pt}
            \item Tanah vulkanik Aceh.
            \item Kondisi lab terkontrol.
            \item Referensi: XRF/ICP-OES.
            \item \textit{Fine-tuning} \& evaluasi.
          \end{itemize}
          }
        \end{column}
      \end{columns}
