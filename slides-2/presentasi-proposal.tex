...
\begin{frame}{Previous Research}
  \centering  
  \tikzset{
    titlebox/.style={font=\normalsize\bfseries\color{maincolor}, align=center, text width=10.4cm},
    flowbox/.style={rectangle, draw=maincolor!50, fill=white, rounded corners=1mm,
      text width=4.8cm, align=center, font=\small, minimum height=1.3cm, inner sep=2mm},
    gapbox/.style={flowbox, draw=red!50, fill=red!5},
    impbox/.style={rectangle, draw=orange!80, fill=orange!5, rounded corners=1mm,
      text width=10.4cm, align=center, font=\small\bfseries, minimum height=0.6cm, inner sep=2mm},
    arrow/.style={-{Stealth[length=3mm,width=2mm]}, thick, draw=maincolor!70}
  }

  \begin{tikzpicture}[node distance=0.2cm and 0.4cm]
    \node[titlebox] (title) {1. Favre et al. (2025) -- MERLIN (\textit{Physics Simulation})};
    
    \node[flowbox, below=0.2cm of title.south west, anchor=north west] (p1) {\textbf{Problem}\\[0.5em] \footnotesize CF-LIBS butuh model plasma realistis untuk pemetaan \textit{spectral emission}.};
...
\begin{frame}{Previous Research}
  ...
  \begin{tikzpicture}[node distance=0.2cm and 0.4cm]
    \node[titlebox] (title) {2. Walidain et al. (2026) -- Informer (\textit{Qualitative DL})};
    
    \node[flowbox, below=0.2cm of title.south west, anchor=north west] (p1) {\textbf{Problem}\\[0.5em] \footnotesize Rentang spektrum emisi LIBS berdimensi sangat besar.};
...
\begin{frame}{Previous Research}
  ...
  \begin{tikzpicture}[node distance=0.2cm and 0.4cm]
    \node[titlebox] (title) {3. Liu et al. (2026) -- transCNN (\textit{Quantitative Hybrid DL})};
    
    \node[flowbox, below=0.2cm of title.south west, anchor=north west] (p1) {\textbf{Problem}\\[0.5em] \footnotesize Analisis Remote LIBS terganggu oleh variansi SNR \& \textit{noise} pola non-linear.};
...
\begin{frame}{Previous Research}
  ...
  \begin{tikzpicture}[node distance=0.2cm and 0.4cm]
    \node[titlebox] (title) {4. Favre et al. (2025) -- ML CF-LIBS (\textit{Quantitative ML})};
    
    \node[flowbox, below=0.2cm of title.south west, anchor=north west] (p1) {\textbf{Problem}\\[0.5em] \footnotesize Butuh iterasi tebakan empiris \textit{real-time} rasio multi-variabel elemen.};
...
% =============================================================
% Slide 5e: Literature Landscape (Box-to-Box Roadmap)
% =============================================================
\begin{frame}{Literature Landscape \& Roadmap}
  \centering
  \vspace{0.8em}
  
  \tikzset{
    litbox/.style={rectangle, rounded corners, draw=maincolor!80, fill=maincolor!10,
      text width=2.4cm, align=center, minimum height=1.4cm, font=\scriptsize\sffamily},
    gapbox/.style={rectangle, rounded corners, draw=red!60, fill=red!10,
      text width=2.4cm, align=center, minimum height=1.4cm, font=\scriptsize\sffamily},
    thesisbox/.style={rectangle, rounded corners, draw=green!60!black, fill=green!10,
      text width=3.5cm, align=center, minimum height=1.6cm, font=\small\sffamily\bfseries},
    arrow/.style={-{Stealth[length=3mm,width=2mm]}, thick, draw=black!50}
  }

  \begin{tikzpicture}[node distance=0.6cm and 0.8cm]
    % Nodes
    \node[litbox] (merlin) {\textbf{Physics Simulation}\\ \vspace{0.2em} {\tiny Favre et al. (2025)}\\ \vspace{0.1em} \textit{\tiny Radiative Transfer LTE}};
    
    \node[litbox, right=of merlin] (informer) {\textbf{Qualitative DL}\\ \vspace{0.2em} {\tiny Walidain et al. (2026)}\\ \vspace{0.1em} \textit{\tiny Informer Transformer}};
    
    \node[litbox, right=of informer] (hybrid) {\textbf{Quantitative DL}\\ \vspace{0.2em} {\tiny Liu / Favre (2025/26)}\\ \vspace{0.1em} \textit{\tiny Hybrid CNN \& Regresi}};
    
    \node[gapbox, below=0.8cm of hybrid] (gap) {\textbf{Unresolved Gap}\\ \vspace{0.2em} \textit{\tiny Single-signal processing lacks structural composition}};

    \node[thesisbox, left=1cm of gap] (thesis) {Proposed Thesis\\ \vspace{0.2em} \textit{\scriptsize Physics-Informed CNN--Transformer}};
    
    % Arrows
    \draw[arrow] (merlin) -- (informer);
    \draw[arrow] (informer) -- (hybrid);
    \draw[arrow] (hybrid) -- (gap);
    \draw[arrow, draw=red!60, very thick] (gap) -- (thesis);
    
  \end{tikzpicture}

  \vspace{1.5em}
  {\small \textbf{Konklusi Peta Riset:}\\ \textit{Pendekatan kualitatif dan kuantitatif masih memperlakukan spektrum secara "black-box". Studi ini memecah belenggu tersebut via dekomposisi eksplisit mono-poliatom.}}
\end{frame}

% =============================================================
% Slide 6: Celah Penelitian
% =============================================================
\begin{frame}{Research Gap}
  ...
  \tikzset{
    box/.style={rectangle, rounded corners, draw=black!40, fill=gray!10,
      text width=3.2cm, align=center, minimum height=1.3cm, font=\small\sffamily},
    gapbox/.style={box, draw=red!60!black, fill=red!8, font=\small\sffamily\bfseries},
    arrow/.style={-{Stealth[length=3mm,width=2mm]}, thick}
  }

  \begin{tikzpicture}[node distance=0.8cm and 1.2cm]
    \node[box] (current) {\textbf{Saat Ini}\\Spektrum $\rightarrow$ Model\\(sinyal tunggal)};
    \node[gapbox, right=of current] (gap) {\textbf{CELAH}\\Hubungan mono--poly\\tidak dimodelkan};
    \node[box, right=of gap] (proposed) {\textbf{Diusulkan}\\Encoder--Decoder\\(komposisi eksplisit)};

    \draw[arrow, draw=red!50] (current) -- (gap);
    \draw[arrow, draw=maincolor] (gap) -- (proposed);
  \end{tikzpicture}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}\setlength{\itemsep}{2pt}
        \item Belum ada pemisahan spektrum monoatom vs poliatom.
        \item Belum ada mekanisme \textit{attention} lintas komposisi.
        \item Belum ada kerangka encoder--decoder untuk LIBS.
      \end{itemize}}
    \end{column}
    \begin{column}{0.48\textwidth}
      {\small
      ...
      \begin{itemize}\setlength{\itemsep}{2pt}
        ...
        \item Decoder mempelajari komposisi poliatom via \textit{cross-attention}.
        \item Prediksi konsentrasi secara \textit{end-to-end}.
      \end{itemize}}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{block}{Rumusan Masalah}
        \begin{enumerate}\setlength{\itemsep}{5pt}
          \item Bagaimana memodelkan hubungan spektral \textbf{mono--poliatom}?
          \item Bagaimana mengekstraksi \textbf{fitur lokal} dan \textbf{dependensi global} secara simultan?
          \item Dapatkah CNN--Transformer \textbf{meningkatkan} analisis kuantitatif LIBS?
        \end{enumerate}
      \end{block}
    \end{column}
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Tujuan Penelitian}
        \begin{enumerate}\setlength{\itemsep}{5pt}
          \item Memodelkan komposisi spektral mono--poliatom melalui encoder--decoder.
          \item Mengembangkan arsitektur CNN--Transformer untuk pola lokal \& global.
          \item Memprediksi konsentrasi unsur pada tanah vulkanik.
          \item Mengevaluasi terhadap metode \textit{baseline}.
        \end{enumerate}
      \end{block}
    \end{column}
  \end{columns}
